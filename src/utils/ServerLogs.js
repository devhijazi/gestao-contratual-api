...
const dateLog = () => moment.locale('pt-BR') && chalk.black.bgWhiteBright(`Data e Hora\n[${moment().format('DD-MM-YYYY hh:mm:ss')}]`);
...
module.exports = class ServerLogs {
    morganLog(req, res, next) {
        return morgan((tokens, req, res) => {
            ...
            return [
                ...
                tokens.url(req, res), '-', 'IP',
                tokens['remote-addr'](req, res), '-', 'Code', '-',
                tokens.status(req, res), '-', 'Size',
    ...
    log(error, msg, opts) {
        ...
        const colorFunction = _.get(
            chalk,[bold,italic,underline,reversed,bgColor,color].filter(Boolean).join(' ')
        )
        console.log(
            isError ? `${dateLog()} ${chalk.black.bgRedBright('[ErrorLog]')}` : dateLog(),
